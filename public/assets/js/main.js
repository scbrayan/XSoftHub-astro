(function ($) {
  ...
  */
  ...
  document.addEventListener('astro:page-load', function() {
    // Hide preloader on page load (for client-side navigation)
    if (typeof $ !== 'undefined') {
      $('.cs-preloader_in').fadeOut();
      $('.cs-preloader').delay(150).fadeOut('slow');
      
      // Re-initialize other functions
      $(window).trigger('scroll');
      $(window).trigger('resize');
      isotopInit();
    }
  ...
  $(function () {
    ...
    // Move the mousemove event listener inside the ready function
    // to ensure cursor elements exist before GSAP tries to animate them
